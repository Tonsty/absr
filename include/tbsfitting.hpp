...
namespace absr {
	...
	template<Degree deg>
	void TBSFitting3L::fitting(TensorBSplines<deg> &tbs) {
		...
		TensorBSplines<deg>::make_data_mat(points_3L, data_mat, N);

		SparseMatrix smooth_mat; 
		TensorBSplines<deg>::make_smooth_mat(smooth_mat, N);
	...
	template<Degree deg>
	void TBSFittingJuttler::fitting(TensorBSplines<deg> &tbs) {
		PointSet &points = points_;
		NormalSet &normals = normals_;
		const Scalar lambda = lambda_;
		...
		TensorBSplines<deg>::make_data_duvw_mat(points, data_mat, du_mat, dv_mat, dw_mat, N);
	...
	template<Degree deg>
	void TBSFittingSDF::fitting(TensorBSplines<deg> &tbs) {
		const SDF &sdf = sdf_;
		const Scalar lambda = lambda_;
		...
		if (global_fitting) {
			...
			SparseMatrix data_mat;
			TensorBSplines<deg>::make_data_mat(points, data_mat, N);
			...
			TensorBSplines<deg>::make_data_1d_mat(points_1d, data_1d_mat, N);

			SparseMatrix smooth_1d_mat; 
			TensorBSplines<deg>::make_smooth_1d_mat(smooth_1d_mat, N);
