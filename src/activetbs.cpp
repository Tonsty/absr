...
Scalar ActiveTBS::operator()(Scalar x, Scalar y, Scalar z) {
	const Size N = N_; 
	const MapType &amp = amp_; 

	std::vector<std::pair<Index, Scalar>> iws;
	TensorBSplines::make_tbs_iws(iws, x, y, z, N);
...
void ActiveTBS::generate_active_map(MapType &amp, const PointSet &points, const Size N) {
...
void ActiveTBS::make_data_mat(const MapType &amp, const PointSet &points, SparseMatrix &data_mat, const Size N) {
	...
	for (Index row_index = 0; row_index < npts; row_index++) {
		...
		TensorBSplines::make_tbs_iws(iws, xpos, ypos, zpos, N);
		for(auto it = iws.begin(); it!= iws.end(); it++) {
...
void ActiveTBS::make_smooth_mat(const MapType &amp, SparseMatrix &smooth_mat, const Size N) {

	std::cerr << "\nprepare smooth_mat:" << std::endl;

	Matrix Pi0, Pi1, Pi2;
	TensorBSplines::precompute_Pi0Pi1Pi2(Pi0, Pi1, Pi2, N);
...
void ActiveTBS::make_data_duvw_mat(const MapType &amp, const PointSet &points, SparseMatrix &data_mat, SparseMatrix &du_mat, 
	SparseMatrix &dv_mat, SparseMatrix &dw_mat, const Size N) {
		...
		for (Index row_index = 0; row_index < npts; row_index++) {
			...
			TensorBSplines::make_tbs_iws_duvw_ws(iws, du_ws, dv_ws, dw_ws, xpos, ypos, zpos, N);
			Index h = 0;
