...
void make_data_mat(const PointSet &points, SparseMatrix &data_mat) {
	...
	std::vector<Triplet> data_tripleList;
	for (Index row_index = 0; row_index < npts; row_index++) {
		...
		for(auto it = iws.begin(); it!= iws.end(); it++) {
			...
			data_tripleList.push_back(Triplet(row_index, column_index, weight));
		}
	}
	data_mat.setFromTriplets(data_tripleList.begin(), data_tripleList.end());
