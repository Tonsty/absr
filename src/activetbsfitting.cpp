...
void ActiveTBSFitting3L::fitting(ActiveTBS &atbs) {
	...
	ActiveTBS::generate_active_map(amp, points_3L, N);

	SparseMatrix data_mat;
	ActiveTBS::make_data_mat(amp, points_3L, data_mat, N);

	SparseMatrix smooth_mat; 
	ActiveTBS::make_smooth_mat(amp, smooth_mat, N);
...
void ActiveTBSFittingJuttler::fitting(ActiveTBS &atbs) {
	...
	ActiveTBS::make_data_duvw_mat(amp, points, data_mat, du_mat, dv_mat, dw_mat, N);
