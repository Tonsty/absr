...
void HierarchicalTBSFitting3L::fitting(HierarchicalTBS &htbs) {
	...
	for (Index level = 0; level < L; level++) {
		...
		ActiveTBS::generate_active_map(amp, points_3L, current_N);

		SparseMatrix data_mat;
		ActiveTBS::make_data_mat(amp, points_3L, data_mat, current_N);

		SparseMatrix smooth_mat; 
		ActiveTBS::make_smooth_mat(amp, smooth_mat, current_N);
...
void HierarchicalTBSFittingJuttler::fitting(HierarchicalTBS &htbs) {
	...
	for (Index level = 0; level < L; level++) {
		...
		Vector &controls = htbs.controls_;
		const Size current_N = (root_N-3)*(1<<level) + 3;
		ActiveTBS::generate_active_map(amp, points, current_N);

		SparseMatrix data_mat, du_mat, dv_mat, dw_mat;
		ActiveTBS::make_data_duvw_mat(amp, points, data_mat, du_mat, dv_mat, dw_mat, current_N);
